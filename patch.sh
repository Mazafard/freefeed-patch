...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/mazafard/patch/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
uninstall() {
    confirm "sure you want to uninstall patch?" "n"
    if [[ $? != 0 ]]; then
        if [[ $# == 0 ]]; then
            show_menu
        fi
    ...
    systemctl disable patch
    rm /etc/systemd/system/patch.service -f
    ...
    echo ""
    echo -e "uninstall patch succeed,you can delete this script by ${green}rm /usr/bin/patch -f${plain}"
    echo ""
...
reset_user() {
    ...
    /usr/local/patch/patch setting -username admin -password admin
    echo -e "your username and password are reset to ${green}admin${plain},restart patch to take effect"
    confirm_restart
}
...
reset_config() {
    ...
    /usr/local/patch/patch setting -reset
    echo -e "all settings are reset to default,please restart patch,and use default port ${green}54321${plain} to access panel"
    confirm_restart
}

check_config() {
    info=$(/usr/local/patch/patch setting -show true)
    if [[ $? != 0 ]]; then
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        /usr/local/patch/patch setting -port ${port}
        echo -e "set port done,please restart patch,and use this new port ${green}${port}${plain} to access panel"
        confirm_restart
    fi
}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        echo ""
        LOGI "patch is running,no need to start agin"
    else
        systemctl start patch
        sleep 2
        check_status
        if [[ $? == 0 ]]; then
            LOGI "start patch  succeed"
        else
            LOGE "start patch failed,please check logs"
        fi
    fi
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        echo ""
        LOGI "patch is stopped,no need to stop again"
    else
        systemctl stop patch
        sleep 2
        check_status
        if [[ $? == 1 ]]; then
            LOGI "stop patch succeed"
        else
            LOGE "stop patch failed,please check logs"
        fi
    fi
...
restart() {
    systemctl restart patch
    sleep 2
    check_status
    if [[ $? == 0 ]]; then
        LOGI "restart patch succeed"
    else
        LOGE "stop patch failed,please check logs"
    fi
    if [[ $# == 0 ]]; then
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "enable patch on system startup succeed"
    else
        LOGE "enable patch on system startup failed"
    fi
...
disable() {
    systemctl disable patch
    if [[ $? == 0 ]]; then
        LOGI "disable patch on system startup succeed"
    else
        LOGE "disable patch on system startup failed"
    fi
...
show_log() {
    journalctl -u patch.service -e --no-pager -f
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
}
...
update_shell() {
    wget -O /usr/bin/patch -N --no-check-certificate https://github.com/mazafard/patch/raw/master/patch.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ ! -f /etc/systemd/system/patch.service ]]; then
        return 2
    fi
    temp=$(systemctl status patch | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    temp=$(systemctl is-enabled patch)
...
check_uninstall() {
    ...
    if [[ $? != 2 ]]; then
        echo ""
        LOGE "patch is installed already"
        if [[ $# == 0 ]]; then
            before_show_menu
        fi
        return 1
...
check_install() {
    ...
    if [[ $? == 2 ]]; then
        echo ""
        LOGE "please install patch first"
        if [[ $# == 0 ]]; then
            before_show_menu
        fi
        return 1
...
show_status() {
    ...
    case $? in
    0)
        echo -e "patch status: ${green}running${plain}"
        show_enable_status
        ;;
    1)
        echo -e "patch status: ${yellow}stopped${plain}"
        show_enable_status
        ;;
    2)
...
show_usage() {
    echo "patch control menu usages: "
    echo "------------------------------------------"
...
show_menu() {
    echo -e "
  ${green}patch control menu${plain}
  ${green}0.${plain} exit
————————————————
  ${green}1.${plain} install   patch
  ${green}2.${plain} update    patch
  ${green}3.${plain} uninstall patch
————————————————
...
————————————————
  ${green}8.${plain} start patch
  ${green}9.${plain} stop  patch
  ${green}10.${plain} restart patch
  ${green}11.${plain} check patch status
  ${green}12.${plain} check patch logs
————————————————
  ${green}13.${plain} enable  patch on system startup
  ${green}14.${plain} disable patch on system startup
————————————————
